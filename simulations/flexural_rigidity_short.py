...
def run_bending_params(k_bending, seed, save_img):
    '''
    Set up canonical actin filament, simulation system, and energy functions.
    '''
    box_l = [750, 750, 750]
    part_num = 200
    camera_position = [box_l[0] / 2, 1500, box_l[2] / 2]
    ...
    folder_param = 'output/flexural_rigidity_short/compression/kb%04d' % k_bending
    ...
    for frame in range(frame_num):
        ...
        if frame % 100 == 0:
            length_current = pos_ic[-1, -1] - pos_ic[0, -1]
            length_initial = 2.78 * (part_num - 1)
    ...
    output = dict(k_bending=k_bending, seed=seed, pos_tic=pos_tic)
...
def scan_bending_params():
    k_bending_set = [45, 145, 945, 1745, 2545]
    seed_set = [1, 2, 3, 4]
    save_img = 1

    for seed in seed_set:
        for k_bending in k_bending_set:
            print('Scanning k_b = %d using seed %d' % (k_bending, seed))
            subprocess.run([
                sys.executable,
                __file__,
                'run-bending-params', # _ is replaced with - by argh
                str(k_bending),
                str(seed),
                str(save_img)
            ], check=True)
...
if __name__ == '__main__':
    argh.dispatch_commands([scan_bending_params, run_bending_params])
