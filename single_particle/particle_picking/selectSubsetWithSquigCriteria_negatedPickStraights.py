...
for i in range(len(file_names)):#8300,8400):
	...
	if(num_lines > 14):
		...
		for j in set(vals[:,3]):# vals[:,3} is the list of filament IDs
			...
			if(not(isSquig_posCurv and isSquig_negCurv)):
				...
				print('Micrograph ' + file_names[i] + ' filament number ' + str(j) + ' is not a squiggle')
...
for key in squigHash.keys():
	newStarName = key[:-5] + '_nonSquigPick.star'
