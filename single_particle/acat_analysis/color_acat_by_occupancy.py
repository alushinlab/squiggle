...
def color_by_occupancy(pdb_file_name, acat_map, idx):
	...
	occ_vals = np.loadtxt('./occupancy_txt/occupancy_frame%s.txt'%str(idx).zfill(2))[::-1]
...
for i in range(0,20):
	pdb_file = '/mnt/data1/ayala/final_squiggle_paper/measure_all/pdbs/acat_3dva/acat_3dva_frame%s.pdb'%str(i)
	mrc_file = '/mnt/data1/ayala/final_squiggle_paper/measure_all/pdbs/acat_3dva/frame_%s.mrc'%str(i).zfill(3)
