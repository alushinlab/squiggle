...
################################################################################
file_names = sorted(glob.glob('../measurements/acat_upToFrame4/acatVols_segmented/*.mrc'))[2:-2]
sums = compute_sum(file_names)
sums_norm = normalize(sums)

sums_cumul = compute_sum_thresh(file_names, 0.025)
